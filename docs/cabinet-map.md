...
- Попутно: PHP-warning `"continue" targeting switch` в
  `JoomlaCoupon.php` – ✅ исправлено W14 (`continue 2`): сводка процентных
  промокодов больше не считает пропускаемые заказы, падение первого
  запроса в свежем PHP-процессе ушло; баланс не изменился (сверено по 38
  партнёрам). Статус 30 → 16 – миграция W14 (`2026_09_25_160000_…`),
  прогоняет владелец.
- Доля скидки в правиле «старых 10%-промокодов» сводки
  (`getUserPercentCouponsSummary()`) считалась от накопителя
  `$totalSubtotal` по предыдущим заказам – ✅ исправлено W15: доля берётся
  от `order_subtotal` своего заказа. Затронуты только заказы нового сайта
  с нулевым начислением (у Joomla-заказов кешбэк уже проставляет
  `applyCashbackFix()`); баланс не изменился (сверено по 38 партнёрам).
...
| В | «Партнёры»: таблица активности + карточка партнёра (колонки – решение 25.09, в промпте) – **PPM-W13** | 10 | ✅ принят 25.09 (раунд 2 проверен в браузере) |
| А2 | Деньги: миграция статуса 30→16, `continue 2` в сводке промокодов, скрипты сверки балансов при выкате – **PPM-W14** (после W13: оба правят `JoomlaCoupon.php`) | – | ✅ принят 25.09 (миграция прогнана владельцем, сверка снимков – только ожидаемые −686 ₽ у #122 и #166) |
...
свободный – **W16**. W15 – доля скидки в сводке промокода
(`docs/prompts/stage-w15-coupon-summary-ratio.md`, решение владельца 25.09).
