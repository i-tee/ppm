...
## 4в. Админский экран «Партнёры» (этап В, 2026-09-25)

**Эндпоинты** (гейт `admin` — бухгалтеру этот экран не нужен;
`app/Http/Controllers/AdminPartnersController.php`):

| Метод | Путь | Что отдаёт |
|---|---|---|
| `GET` | `/api/admin/partners` | список партнёров с метриками; параметры `q` (часть имени/email), `status_id` (статус анкеты), `activity`, `sort`, `dir`, `page`, `per_page` (≤100), `refresh=1` (собрать в обход кеша) |
| `GET` | `/api/admin/partners/{id}` | карточка партнёра (`whereNumber`); `refresh=1` — в обход кеша |

Партнёры = пользователи без уровней доступа 1/2/3
(`whereDoesntHave('accessLevels')`).

**Одна формула баланса.** Расчёт вынесен из
`UserCouponController::buildBusinessData()` в
`App\Services\PartnerBalanceCalculator::compute()`. Кабинет партнёра и
админский список зовут один и тот же метод, отличается только сборка
заказов, поэтому баланс, «всего заработано» и число оплаченных заказов в
списке и у самого партнёра совпадают. Порядок операций внутри
калькулятора менять нельзя (в том числе `ceil()` на первом шаге) — иначе
цифры разъедутся со старым кабинетом.

**Сколько запросов.** Число запросов у списка **не растёт** с числом
партнёров — растёт только количество раундов в бэкенд:

- своя БД — 5 выборок на всех сразу (пользователи, анкеты, проверенные
  реквизиты, заявки на вывод, бонус-коды);
- Joomla — 5 выборок на всех сразу (`jm_users` по email `IN`,
  `avicenna_user_coupons` по `user_id IN`, `jshopping_coupons` и
  `jshopping_orders` по `coupon_id IN`, `avicenna_pp_payments` по
  `user_id IN`). Заказы — только белым списком полей
  (`JoomlaCoupon::orderSafeFields()`), без ПДн покупателя, см. §5;
- основной бэкенд — `AvicennaBackendClient::getAccrualsRedemptionsBatch()`
  на `Http::pool`, чанками по 5 партнёров (×2 эндпоинта = 10
  одновременных запросов), доборы страниц — следующими раундами, предел
  страниц и флаг `PARTNER_ACCRUALS_FROM_BACKEND` те же, что у одиночных
  методов.

Замер на локалке (46 партнёров, обе БД — удалённый Beget): холодная
сборка списка ~8.5–9.0 с, из кеша ~1.3–1.4 с (порожний авторизованный
роут в этом окружении — ~1.6 с). Карточка: холодная ~5.4 с, из кеша
~2.2–2.5 с. Время холодной сборки списка упирается в раунды `Http::pool`
(⌈N/5⌉ раундов), поэтому на ~500 партнёрах ожидается ~1 минута — если
столько партнёров станет реальностью, увеличивать надо именно
`AvicennaBackendClient::POOL_PARTNERS`, а не число SQL.

**Кеш списка** — `file`-стор, ключ `ppm:partners-list`, 5 минут. Поиск,
фильтры, сортировка и пагинация считаются на сервере поверх
закешированного массива. Карточка переиспользует общий
`BusinessDataCache` (ключ партнёра, 60 с) — поэтому админ видит ровно те
цифры, что и сам партнёр.

**Если данные не догрузились** (ошибка/таймаут/429 основного бэкенда,
сбой БД) — неполные цифры не показываем. В списке у такого партнёра
денежные ячейки — «Ошибка загрузки», над таблицей — «Не удалось
загрузить часть данных. Попробуйте обновить» с кнопкой «Обновить»
(пересборка в обход кеша); **такой список не кешируется**. В карточке
вместо денежных блоков — «Ошибка при загрузке данных, попробуйте снова»
с той же кнопкой, и частичный `business-data` в общий кеш не попадает.
Слова «новый сайт», «бэкенд», «Joomla» в интерфейс не выносятся —
подробности сбоя (партнёр, эндпоинт, код ответа) уходят только в лог
(`avicenna_backend.batch_*`, `admin.partners_joomla_failed`,
`admin.partner_card_failed`).

**Активность.** Признак считается по дате последнего оплаченного заказа
(оба источника — Joomla и новый сайт):

- «Активен» — оплаченный заказ за последние `active_days` дней;
- «Затих» — заказы были, но не за этот срок;
- «Не начал» — ни одного оплаченного заказа.

Порог — `config/settings.json` → `partner_activity.active_days`
(сейчас 60), не в коде.

**Карточка под админом.** `JoomlaCoupon` берёт пользователя из `Auth`,
поэтому карточка подменяет его на партнёра через `Auth::setUser()` строго
в `try/finally` с гарантированным возвратом админа. По обе стороны
подмены зовётся `JoomlaCoupon::resetRequestCaches()` — часть мемо-кешей
модели (`$backendCache`, `$ppOrdersCache`) глобальная, не по id
пользователя, и без сброса данные протекли бы от одного пользователя к
другому. Если Joomla-пользователя у партнёра нет, `buildBusinessData()`
**не зовётся вовсе** (`getUserCoupons()` умеет создать запись в Joomla, а
просмотр не должен ничего создавать): карточка показывает анкету,
реквизиты, «Войти как партнёр» и статус «Не начал».
...
## 4г. Сверка балансов при выкате (этап А2, 2026-09-25)

Два скрипта в `tools/`, оба запускаются через tinker и **только читают**:

| Скрипт | Запуск | Что делает |
|---|---|---|
| `tools/balance-snapshot.php` | `php artisan tinker tools/balance-snapshot.php` | Снимает деньги всех партнёров с одобренной заявкой в JSON: `balance`, `credits.total_accruals`/`orders_count`, `withdrawals.debit`, `payoutRequests.debit`, суммы и число заявок на выплату **по статусам**, `trueBonusCode.totalBonusCodesCost`, `backendReversals.debit`, плюс общий свод `payout_requests` по статусам |
| `tools/balance-compare.php` | `PPM_SNAPSHOT_A=<до> PPM_SNAPSHOT_B=<после> php artisan tinker tools/balance-compare.php` | Сравнивает два снимка, печатает таблицу «партнёр / поле / было / стало / разница» и итог «расхождений нет» / «N расхождений». Ожидаемые расхождения миграции статуса `30 → 16` — отдельным блоком с итоговой суммой |

- Файл вывода снимка — переменная `PPM_SNAPSHOT_OUT`, по умолчанию
  `storage/app/balance-snapshot-<дата>_<время>.json`. Компаратор без
  переменных берёт два самых свежих снимка из `storage/app`.
- **Только партнёры с одобренной заявкой.** У остальных нет
  Joomla-пользователя, а `JoomlaCoupon::getUserCoupons()` умеет его
  создать — это запись в боевую Joomla, при снятии снимка недопустимая.
  Больше скрипт никуда не пишет: в свою БД — ни одного `INSERT`/`UPDATE`.
- **Кеши.** Перед каждым партнёром сбрасывается `BusinessDataCache`
  (иначе в снимок попадут цифры возрастом до 60 с) и статические
  мемо-кеши `JoomlaCoupon` — они не ключуются по пользователю и в одном
  процессе протекают от партнёра к партнёру (урок этапа В, см. §4в).
- **Совместимость со старым кодом.** Снимок «до» снимается на проде
  *до* `git pull`, поэтому скрипт опирается только на то, что есть и в
  старом коде (коммит `8219482`), и в новом: `UserCouponController::data(Request)`,
  возвращающий `JsonResponse`, `Auth::login()` + `setUserResolver()`.
  Метода `JoomlaCoupon::resetRequestCaches()` в старом коде нет — там
  статические кеши гасятся рефлексией (`method_exists` → иначе
  рефлексия), а `BusinessDataCache` подключается только через
  `class_exists`. В старом коде первая компиляция `JoomlaCoupon.php`
  бросает `ErrorException` (`continue` в `switch`, исправлено этапом А2) —
  скрипт прогревает класс до цикла и этот прогрев снимком не считает.
- Порядок действий на проде — `docs/rollout-ux.md` §4а.
